...
#' 
#' result.cv <- FPgaMsgl.cv(lowD$X, lowD$Y, lowD$B0, model="L121", lowD$Gm, lowD$mi, mg.v, mc.v, fold=5, seed=1, ncores=4)
#' 
...
#' system.time(try3 <- FPgaMsgl(lowD$X, lowD$Y, lowD$B0, model="L121", lowD$Gm, lowD$mi, grp.max.cv.rmse, coe.max.cv.rmse))
#' system.time(try4 <- FPgaMsgl(lowD$X, lowD$Y, lowD$B0, model="L121", lowD$Gm, lowD$mi, grp.max.cv.r2, coe.max.cv.r2))
#' 
#' @useDynLib PgaMsgl
#' 
...
#' 
#' @export
FPgaMsgl.cv <- function(XX, YY, B0, model=c("L020v1", "L020v2", "L121"), Gm, mi=1000, mg.v, mc.v, minlambda = 1e-5, rlambda = 0.98, mintau = 1e-5, rtau = 0.98, fold=5, seed=1, ncores=NULL)
{
  ...
  cv <-
    ...
    foreach(j=1:(n.mg.v*n.mc.v), .combine='ccom', .options.mpi=opt) %dopar% 
    {
      ...
      if (model == "L020v1"){
        result <- FPga_L020v1(X.train, Y.train, B0, Gm, mi, grp.max, coe.max)
      } else if (model == "L020v2") {
        result <- FPga_L020v2(X.train, Y.train, B0, Gm, mi, grp.max, coe.max, minlambda, rlambda, mintau, rtau)
      } else if (model == "L121") {
        result <- FPga_L121(X.train, Y.train, B0, Gm, mi, grp.max, coe.max)
      } else {
